...
wheel_momentum = np.array([0, 0, wheel_inertia * wheel_omega])
...
for axis in [0, 1, 2]:
    ...
    perturbation = 0.0 #0.1 * omega_magnitude * np.deg2rad( np.random.rand(3) );
    ...
    while now <= duration:
        ...
        sc.propagate_attitude_with_a_wheel(
            timestep, [0,0,0], wheel_inertia, wheel_omega, 0.0, [0,0,1])
